...
Companion services for the customized Pluely fork. None of this is required to run Pluely — it's an opt-in stack that adds:

- **`proxy.py`** — OpenAI-compatible HTTP server on `127.0.0.1:8765` that forwards chat requests to the Anthropic Claude Code CLI (`claude -p`). Lets you point Pluely's AI provider at a local URL and use your Claude login instead of paying per Anthropic API token. Supports streaming and screenshot attachments.
...
### 3. Node tools for writing Pluely's LevelDB

The `level-tools/` scripts configure Pluely's custom AI / STT providers by writing directly to its Chromium LevelDB. Skipping the UI entirely.
...
### 6. Configure Pluely's providers

Run these once. They write the Chromium LocalStorage keys Pluely reads on startup. Pluely should be **closed** while these run.
...
The STT key in Pluely's storage is called `custom-local-whisper` for historical reasons — it actually points at whichever server is listening on 8766, so both `google-stt-server.py` and `whisper-server.py` are interchangeable.

## Use

| Shortcut on Desktop | What it runs |
| --- | --- |
| `Pluely (Claude).lnk` | `start-pluely-claude.cmd` — selects the Claude proxy AI provider, starts `proxy.py` (8765) and `google-stt-server.py` (8766) if not already up, launches Pluely. |
| `Pluely (Ollama).lnk` | `start-pluely-ollama.cmd` — selects the Ollama AI provider, starts `google-stt-server.py` (8766), launches Pluely. (No proxy needed — Pluely talks to Ollama directly on 11434.) |
...
foreach ($pair in @(
    @{ name = 'Pluely (Claude).lnk';      target = "$env:USERPROFILE\pluely-proxy\start-pluely-claude.cmd" },
    @{ name = 'Pluely (Ollama).lnk';      target = "$env:USERPROFILE\pluely-proxy\start-pluely-ollama.cmd" },
...
| `level-tools/install-provider.mjs` | Install custom Claude proxy AI provider into Pluely's LevelDB |
| `level-tools/install-speech-provider.mjs` | Install custom STT provider into Pluely's LevelDB |
...
| `level-tools/dump-keys.mjs` | Debug: enumerate Pluely's LevelDB keys/values |
...
- `*.log` — runtime logs
- `leveldb-backup-*/` — snapshots of Pluely's LevelDB before destructive writes
